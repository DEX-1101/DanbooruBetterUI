 ...
 */
export const getPostsByTag = async (tagName: string, safeMode: boolean = true, page: number = 1): Promise<{ posts: Post[], hasMore: boolean }> => {
  try {
    ...
    const limit = clientTags.length > 0 ? 200 : 50;
    
    const res = await fetch(`${BASE_URL}/posts.json?tags=${encodeURIComponent(tagsParam)}&limit=${limit}&page=${page}`);
    if (!res.ok) {
